...
void getMeanSquaredError(float *, float *, int, int, int, float *);
void getCoefficientOfDetermination(float *, float *, int, float *);
void getAdjustedCoefficientOfDetermination(float *, float *, int, int, int, float *);
...
*
* @param float *MSE - This argument will contain the pointer to a
...
*/
void getMeanSquaredError(float *realOutputMatrix, float *predictedOutputMatrix, int n, int m, int degreesOfFreedom, float *MSE) {
...
*				 the output data was obtained.
*
* @param float *Rsquared - This argument will contain the pointer to a
...
* LAST UPDATE: DECEMBER 04, 2021
*/
void getCoefficientOfDetermination(float *realOutputMatrix, float *predictedOutputMatrix, int n, float *Rsquared) {
...
* LAST UPDATE: DECEMBER 04, 2021
*/
void getAdjustedCoefficientOfDetermination(float *realOutputMatrix, float *predictedOutputMatrix, int n, int m, int degreesOfFreedom, float *adjustedRsquared) {
	// We obtain the sums required for the means to be calculated and the SSE values for each of the columns of the input matrix.
	float squareThisValue; // Variable used to store the value that wants to be squared, for performance purposes.
	float mean_realOutputMatrix = 0; // This variable is used to store the means of all the outputs of the argument pointer variable "realOutputMatrix".
	for (int currentRow = 0; currentRow < n; currentRow++) {
