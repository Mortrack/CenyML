...
void getMean(float *, int, int, float *);
void getSort(char [], int, int, float *);
static void applyQuickSort(int, int, float *);
void getMedian(char [], float *, int, int, float *);
void getVariance(float *, int, int, int, float *);
void getStandardDeviation(float *, int, int, int, float *);
void getQuickMode(char [], int, int, float *, int *, float *);
void getMeanIntervals(float *, float *, float, char, int, int, float *);
...
* 
* @param float *inputMatrix - This argument will contain the
...
*
* @param float *mean - This argument will contain the pointer to
...
*						VARIABLE SIZE OF "m" 'float' MEMORY SPACES.
*
...
*/
void getMean(float *inputMatrix, int n, int m, float *mean) {
...
void getSort(char desiredSortMethod[], int n, int m, float *inputMatrix) {
	...
	if (strcmp(desiredSortMethod, "quicksort") == 0) {
		...
		float *rowsOfCurrentColumn = (float *) malloc(n*sizeof(float));
...
*							 current "quick sort" requested.
*
* @param float *inputVector - This argument will contain the
*							   pointer to a memory allocated input
*							   vector, from which the "quick sort"
*							   method will be applied. THIS VARIABLE
*							   SHOULD BE ALLOCATED AND INNITIALIZED
*							   BEFORE CALLING THIS FUNCTION WITH
*							   A SIZE OF "n" 'float' MEMORY SPACES.
*
* NOTE: RESULT IS STORED IN THE MEMORY ALLOCATED POINTER VARIABLE
*       "inputVector".
...
* LAST UPDATE: DECEMBER 04, 2021
*/
static void applyQuickSort(int minIndexLimit, int maxIndexLimit, float *inputVector) {
	...
	float pivotValue = inputVector[(indexOfLeftElement + indexOfRightElement)/2];
	float exchangeValue;
...
*				 matrix has, with which the output data was obtained.
*
* @param float *Q2 - This argument will contain the pointer to
*					  a memory allocated variable in which we
*					  will store the median for each column
*					  contained in the argument variable
*					  "inputMatrix". IT IS INDISPENSABLE THAT THIS
* 					  VARIABLE IS ALLOCATED BEFORE CALLING THIS
*					  FUNCTION WITH A SIZE OF "m" 'float' MEMORY
*					  SPACES.
*
...
* LAST UPDATE: DECEMBER 04, 2021
*/
void getMedian(char desiredSortMethod[], float *inputMatrix, int n, int m, float *Q2) {
...
*
* @param float *variance - This argument will contain the pointer to
*							a memory allocated variable in which we
...
* LAST UPDATE: DECEMBER 04, 2021
*/
void getVariance(float *inputMatrix, int n, int m, int degreesOfFreedom, float *variance) {
	// We declare and innitialize the variable "mean" with "0"s. The mean for each column will be stored in this variable.
	float mean[m]; // The mean of each column will be stored in this variable.
	for (int currentColumn = 0; currentColumn < m; currentColumn++) {
...
*								deviation.
*
* @param float *standardDeviation - This argument will contain the
*									 pointer to a memory allocated
...
* LAST UPDATE: DECEMBER 04, 2021
*/
void getStandardDeviation(float *inputMatrix, int n, int m, int degreesOfFreedom, float *standardDeviation) {
	// We declare and innitialize the variable "mean" with "0"s. The mean for each column will be stored in this variable.
	float mean[m]; // The mean of each column will be stored in this variable.
	for (int currentColumn = 0; currentColumn < m; currentColumn++) {
		mean[currentColumn] = 0;
...
*                    SIZE OF "n" 'float' MEMORY SPACES.
*
* @param float *Mo - This argument will contain the pointer to a
...
*					  A SIZE OF "n" TIMES "m" 'float' MEMORY SPACES.
*
...
*/
//TODO: Test the performance of the "quick sort" method when argumenting the entire matrix in "applyQuickSort()" instead of several vectors, to compare it with the current method.
void getQuickMode(char desiredSortMethod[], int n, int m, float *inputMatrix, int *Mo_n, float *Mo) {
	// If the implementer requested the "quick sort" method, then apply it through the following code.
	if (strcmp(desiredSortMethod, "quicksort") == 0) {
		...
		float *rowsOfCurrentColumn = (float *) malloc(n*sizeof(float)); // This pointer variable will be used as the vector containing the rows of a certain column from the "inputMatrix".
		float *counterOfInputVector = (float *) calloc(n, sizeof(float)); // This pointer variable will be used as a repetition counter for the mode to be calculated.
...
*						VALUES AND A VARIABLE SIZE OF "m" 'float'
*						MEMORY SPACES.
*
* @param float *standardDeviation - This argument will contain the
...
*				 matrix has, with which the output data was obtained.
*
* @param float *meanIntervals - This argument will contain the pointer
...
* LAST UPDATE: N/A
*/
void getMeanIntervals(float *mean, float *standardDeviation, float desiredTrustInterval, char isTdistribution, int n, int m, float *meanIntervals) {
	...
	float areaUnderNormalCurve; // This variable will store the area under the normal curve (which will be either with the "z" or the "t" distribution).
	if (isTdistribution == 1) {
		...
		if (desiredTrustInterval == 95) { // If the desired trust interval is 95%, then get the following t distribution (t_{alpha/2}).
			float tValue[] = {12.706,4.303,3.182,2.776,2.571,2.447,2.365,2.306,2.262,2.228,2.201,2.179,2.160,2.145,2.131,2.120,2.110,2.101,2.093,2.086,2.080,2.074,2.069,2.064,2.060,2.056,2.052,2.048,2.045,2.042};
			...
			float tValue[] = {63.656,9.925,5.841,4.604,4.032,3.707,3.499,3.355,3.250,3.196,3.106,3.055,3.012,2.977,2.947,2.921,2.898,2.878,2.861,2.845,2.831,2.819,2.807,2.797,2.787,2.779,2.771,2.763,2.756,2.750};
			areaUnderNormalCurve = tValue[n-2]; // We retrieve and store the corresponding "t" value.
		} else { // If the desired trust interval is 99.9%, then get the following t distribution (t_{alpha/2}).
			float tValue[] = {636.578,31.600,12.924,8.610,6.869,5.959,5.408,5.041,4.781,4.587,4.437,4.318,4.221,4.140,4.073,4.015,3.965,3.922,3.883,3.850,3.819,3.792,3.768,3.745,3.725,3.707,3.689,3.674,3.660,3.646};
			areaUnderNormalCurve = tValue[n-2]; // We retrieve and store the corresponding "t" value.
		}
	} else {
	...
	float squareRootOfN = sqrt(n); // This variable is used to store a repetitively used value, for performance purposes.
