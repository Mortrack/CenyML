...
static void (*activationFunctions[])(void *) = {getReluActivation_singleGPU, getTanhActivation_singleGPU, getLogisticActivation_singleGPU, getRaiseToTheFirstPowerActivation_singleGPU, getRaiseToTheSecondPowerActivation_singleGPU, getRaiseToTheThirdPowerActivation_singleGPU, getRaiseToTheFourthPowerActivation_singleGPU, getRaiseToTheFifthPowerActivation_singleGPU, getRaiseToTheSixthPowerActivation_singleGPU, getFirstOrderDegreeExponentialActivation_singleGPU, getSecondOrderDegreeExponentialActivation_singleGPU};
...
static void (*derivateOfActivationFunctions[])(void *) = {getDerivateReluActivation_singleGPU, getDerivateTanhActivation_singleGPU, getDerivateLogisticActivation_singleGPU, getDerivateRaiseToTheFirstPowerActivation_singleGPU, getDerivateRaiseToTheSecondPowerActivation_singleGPU, getDerivateRaiseToTheThirdPowerActivation_singleGPU, getDerivateRaiseToTheFourthPowerActivation_singleGPU, getDerivateRaiseToTheFifthPowerActivation_singleGPU, getDerivateRaiseToTheSixthPowerActivation_singleGPU, getDerivateFirstOrderDegreeExponentialActivation_singleGPU, getDerivateSecondOrderDegreeExponentialActivation_singleGPU};
	
struct singleGpuData {
	struct singleNeuronDnnStruct_singleGPU neuronData; // structure variable that will contain all the input data that was previously given to the neuron.
...
void getSingleNeuronDNN_singleGPU(struct singleNeuronDnnStruct_singleGPU *neuron) {
	...
	// Set up the execution configuration that will be desired for the selected GPU.
	dim3 block(32);
	dim3 grid_m((neuron->n * neuron->m + block.x - 1) / block.x);
	...
	CHECK(cudaMalloc((void**) &d_w_best, mPlusOne*sizeof(double)));
	CHECK(cudaMalloc((void**) &d_w_new, mPlusOne*sizeof(double)));
	CHECK(cudaMalloc((void**) &d_TransposeOf_X_tilde, neuron->m*neuron->n*sizeof(double)));
	...
	CHECK(cudaMalloc((void**) &d_w_old, mPlusOne*sizeof(double)));
	CHECK(cudaMalloc((void**) &d_errorTerm, neuron->n*sizeof(double)));
	CHECK(cudaMalloc((void**) &d_errorTerm_dot_Xtilde, mPlusOne*grid_m.x*sizeof(double)));
	
	// We transfer the data that the GPU will need when it starts to operate.
	...
	//CHECK(cudaMemcpy(d_w_best, neuron->w_best, (mPlusOne*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_w_new, neuron->w_new, (mPlusOne*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_f_x_tilde, neuron->f_x_tilde, (neuron->n*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_A_u, neuron->A_u, (neuron->n*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_dA_u, neuron->dA_u, (neuron->n*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_w_old, neuron->w_old, (mPlusOne*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_errorTerm, neuron->errorTerm, (neuron->n*sizeof(double)), cudaMemcpyHostToDevice));
	//CHECK(cudaMemcpy(d_errorTerm_dot_Xtilde, neuron->errorTerm_dot_Xtilde, (mPlusOne*grid_m.x*sizeof(double)), cudaMemcpyHostToDevice));
	...
	for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We create the specified threads and assign them the function they will work with.
		pthread_create(&threadId[currentThread], NULL, getTransposeOfInputData_singleGPU, &threadData[currentThread]);
	}
	...
	for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We create the specified threads and assign them the function they will work with.
		pthread_create(&threadId[currentThread], NULL, getFxTilde_Au_dAu_and_accuracyTermsPart1_singleGPU, &threadData[currentThread]);
	}
	for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We force the program to wait until all threads have finished their assigned task.
		pthread_join(threadId[currentThread], NULL);
	}
	...
	for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We create the specified threads and assign them the function they will work with.
		pthread_create(&threadId[currentThread], NULL, getNeuronAdjustedCoefficientOfDetermination_singleGPUvoidPart2, &threadData[currentThread]);
	}
	for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We force the program to wait until all threads have finished their assigned task.
		pthread_join(threadId[currentThread], NULL);
	}
	...
	for (int currentEpoch=0; currentEpoch<(neuron->maxEpochs); currentEpoch++) {
		...
		for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We create the specified threads and assign them the function they will work with.
			pthread_create(&threadId[currentThread], NULL, getErrorAndUpdateWeightValues_singleGPU, &threadData[currentThread]);
		}
		for(int currentThread=0; currentThread<(neuron->cpuThreads); currentThread++) { // We force the program to wait until all threads have finished their assigned task.
			pthread_join(threadId[currentThread], NULL);
		}
...
/**
* The "getTransposeOfInputData_singleGPU()" static function is used
...
* LAST UPDATE: N/A
*/
static void *getTransposeOfInputData_singleGPU(void *threadVariable) {
...
/**
* The "getFxTilde_Au_dAu_and_accuracyTermsPart1_singleGPU()" static
...
* LAST UPDATE: N/A
*/
static void *getFxTilde_Au_dAu_and_accuracyTermsPart1_singleGPU(void *threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
	...
	getNeuronAdjustedCoefficientOfDetermination_singleGPUvoidPart1(threadVariable); // We calculate the part 1 of the calculation of the current adjusted coefficient of determination of the neuron.
	
	return NULL;
}
/**
* The "getErrorAndUpdateWeightValues_singleGPU()" static function is
...
* LAST UPDATE: N/A
*/
static void *getErrorAndUpdateWeightValues_singleGPU(void *threadVariable) {
	/*
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
* 9) getRaiseToTheSixthPowerActivation_singleGPU() --> Applies the raise to the 6th power activation function.
...
* 9) getDerivateRaiseToTheSixthPowerActivation_singleGPU() --> Derivative of raise to the 6th power activation function.
...
static void getReluActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateReluActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getTanhActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateTanhActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getLogisticActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateLogisticActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getRaiseToTheFirstPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateRaiseToTheFirstPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getRaiseToTheSecondPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateRaiseToTheSecondPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getRaiseToTheThirdPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateRaiseToTheThirdPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getRaiseToTheFourthPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateRaiseToTheFourthPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getRaiseToTheFifthPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateRaiseToTheFifthPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getRaiseToTheSixthPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateRaiseToTheSixthPowerActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getFirstOrderDegreeExponentialActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateFirstOrderDegreeExponentialActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getSecondOrderDegreeExponentialActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
static void getDerivateSecondOrderDegreeExponentialActivation_singleGPU(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
* LAST UPDATE: N/A
*/
static void getNeuronAdjustedCoefficientOfDetermination_singleGPUvoidPart1(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
...
* LAST UPDATE: N/A
*/
static void *getNeuronAdjustedCoefficientOfDetermination_singleGPUvoidPart2(void* threadVariable) {
	// We create a structure variable to access the data that was assigned for the current CPU thread.
	struct singleGpuData* threadData = (struct singleGpuData*) threadVariable;
