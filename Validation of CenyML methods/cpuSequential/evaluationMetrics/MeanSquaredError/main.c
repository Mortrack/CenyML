...
int main(int argc, char **argv) {
	...
	int degreesOfFreedom = 1; // Desired degrees of freedom to be applied in the evaluation metric to be calculated. A "0" would represent a degrees of freedom of "n", a "1" would represent a "n-1", ..., a "degrees" would represent a "n-degrees".
	...
	for (int currentRow=0; currentRow<n; currentRow++) {
		for (int currentColumn=0; currentColumn<p; currentColumn++) {
			Y[currentColumn + currentRow*p] = csv1.allData[columnIndexOfOutputDataInCsvFile + currentColumn + currentRow*p];
		}
	}
	...
	for (int currentRow=0; currentRow<n; currentRow++) {
		for (int currentColumn=0; currentColumn<p; currentColumn++) {
			Y_hat[currentColumn + currentRow*p] = csv2.allData[columnIndexOfOutputDataInCsvFile + currentColumn + currentRow*p];
		}
	}
	elapsedTime = seconds() - startingTime; // We obtain the elapsed time to innitialize the input data to be used.
