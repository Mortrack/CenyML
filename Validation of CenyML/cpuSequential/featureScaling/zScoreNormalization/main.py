...
import time
from sklearn.preprocessing import StandardScaler # version 1.0.1
...
dataset_CenyML_ZscoreNormalizationResults = pd.read_csv('CenyML_getZscoreNormalization_Results.csv')
...
desired_m = len(dataset_CenyML_ZscoreNormalizationResults.iloc[0])
...
# -------------------------------------------------- #
print("Innitializing scikit-learn Z score normalization method calculation ...")
startingTime = time.time()
x_dot = StandardScaler().fit_transform(X) # We apply the desired feature scaling method.
elapsedTime = time.time() - startingTime
print("scikit-learn Z score normalization method elapsed " + format(elapsedTime) + " seconds.")
print("")
...
for currentRow in range(0, len(dataset_mPES100S100SPAPS.iloc[:])):
    for currentColumn in range(0, len(dataset_mPES100S100SPAPS.iloc[0]) ):
        differentiation = abs(dataset_CenyML_ZscoreNormalizationResults.iloc[currentRow][currentColumn] - x_dot[currentRow][currentColumn])
