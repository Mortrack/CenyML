...
// ------------------------------------------------- //
#include <iostream>
#include <dlib/mlp.h> // Dlib library version 19.22
#include <stdio.h>
#include <stdlib.h>
...
int main() {
	...
	char nameOfTheCsvFile2[] = "Dlib_singleNeuron_evalMetrics.csv"; // Name the .csv file that will store the resulting evaluation metrics for the ML model to be obtained.
	...
	typedef matrix<double, 1, 1> sample_type; // DLIB COMMENT: This typedef declares a matrix with 2 rows and 1 column, where the rows represent the number of ML feautres and columns represents the current sample.
	sample_type sample; // DLIB COMMENT: Make an instance of a sample matrix so we can use it below
