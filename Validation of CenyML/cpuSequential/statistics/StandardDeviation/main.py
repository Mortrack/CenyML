...
# standard deviation (https://github.com/numpy/numpy) and then it will be
# compared with the results that were obtained with the CenyML Library as a
# means of validating that the code created in this library for the standard
# deviation method is correct.
# --------------------------------------------------------------------------- #
# --------------------------------------------------------------------------- #
...
dataset_CenyML_standardDeviationResults = pd.read_csv('CenyML_getStandardDeviation_Results.csv')
...
desired_m = len(dataset_CenyML_standardDeviationResults.iloc[0])
...
# -------------------------------------------- #
print("Innitializing NumPy standard deviation method calculation ...")
startingTime = time.time()
standardDeviations = np.std(X, axis=0, ddof=1)
elapsedTime = time.time() - startingTime
print("NumPy standard deviation method elapsed " + format(elapsedTime) + " seconds.")
print("")
...
for currentColumn in range(0, len(dataset_mPES100S100SPAPS.iloc[0]) ):
    differentiation = abs(dataset_CenyML_standardDeviationResults.iloc[0][currentColumn] - standardDeviations[currentColumn])
