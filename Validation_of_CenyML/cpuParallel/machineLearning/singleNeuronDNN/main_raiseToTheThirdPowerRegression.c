...
int main(int argc, char **argv) {
	...
	char nameOfTheCsvFile1[] = "CenyML_raiseToThirdPowerRegression_Coefficients.csv"; // Name the .csv file that will store the resulting coefficient values.
	char nameOfTheCsvFile2[] = "CenyML_raiseToThirdPowerRegression_EvalMetrics.csv"; // Name the .csv file that will store the resulting evaluation metrics for the ML model to be obtained.
	...
	neuron1.activationFunctionToBeUsed = 5; // This variable tells the neuron what activation function to use (see the commented documentation in the function "getSingleNeuronDNN()" for more details).
	neuron1.learningRate = 0.00000000000000000001; // This variable stores the desired learning rate for the neuron to be created.
	neuron1.stopAboveThisAccuracy = 0.99; // The value of this variable is used as a stop function for the single neuron in DNN learning procces.
	...
	for (int currentRow=0; currentRow<neuron1.n; currentRow++) { // Since neuron1.m=neuron1.p=1 for this particular ML algorithm, both "neuron1.Y" and "neuron1.X" will be innitialized here.
		...
		neuron1.Y[currentRow] = (b_ideal[0] + b_ideal[1] * neuron1.X[currentRow]) * (b_ideal[0] + b_ideal[1] * neuron1.X[currentRow]) * (b_ideal[0] + b_ideal[1] * neuron1.X[currentRow]);
	}
