...
* Subsequently, a linear Kernel machine classification method will be
...
* then apply the linear Kernel machine classification on the input and output
...
int main(int argc, char **argv) {
	...
	char csv1Directory[] = "../../../../databases/classification/linearEquationSystem/100systems_10samplesPerAxisPerSys.csv"; // Directory of the reference .csv file
	char nameOfTheCsvFile1[] = "CenyML_trainLinearKernel_Coefficients.csv"; // Name the .csv file that will store the resulting coefficient values.
	char nameOfTheCsvFile2[] = "CenyML_trainLinearKernel_evalMetrics.csv"; // Name the .csv file that will store the resulting evaluation metrics for the ML model to be obtained.
	...
	startingTime = seconds(); // We obtain the reference time to count the elapsed time to apply the simple linear Kernel machine classification with the input data (X).
	// Allocate the memory required for the variable "coefficients", which will contain the identified best fitting coefficient values that will result from the simple linear Kernel machine classification algorithm.
	double *coefficients = (double *) calloc((m+3)*p, sizeof(double));
	// We apply the simple linear Kernel machine classification algorithm with respect to the input matrix "X" and the result is stored in the memory location of the pointer "coefficients".
	getKernelMachineClassification(X, Y_tilde, n, m, p, 0, 0, Kernel, 0, 0, 0, coefficients);
	elapsedTime = seconds() - startingTime; // We obtain the elapsed time to apply the simple linear Kernel machine classification with the input data (X).
	printf("CenyML simple linear Kernel machine classification algorithm elapsed %f seconds.\n\n", elapsedTime);
    ...
    if(success){
        ...
        WriteToFile(pngdata, length, "main_linearKernel (CenyML).png");
