...
#include "../../../../CenyML_library_skeleton/CenyML_Library/cpuSequential/machineLearning/CenyMLclassification.h" // library to use the classification algorithms of CenyML.
...
int main(int argc, char **argv) {
	...
	char isInteractionTerms = 0; // This flag variable determines with a "(int) 1" that interaction terms are desired in the polynomial Kernel and "(int) 0" for otherwise.
	...
	startingTime = seconds(); // We obtain the reference time to count the elapsed time to apply the simple polynomial Kernel machine classification with the input data (X).
	// Allocate the memory required for the variable "coefficients", which will contain the identified best fitting coefficient values that will result from the simple polynomial Kernel machine classification algorithm.
	double *coefficients = (double *) calloc((m*N+3)*p, sizeof(double));
	// We apply the simple polynomial Kernel machine classification algorithm with respect to the input matrix "X" and the result is stored in the memory location of the pointer "coefficients".
	getKernelMachineClassification(X, Y_tilde, n, m, p, N, 0, Kernel, isInteractionTerms, 0, 0, coefficients);
	elapsedTime = seconds() - startingTime; // We obtain the elapsed time to apply the simple polynomial Kernel machine classification with the input data (X).
	printf("CenyML simple polynomial Kernel machine classification algorithm elapsed %f seconds.\n\n", elapsedTime);
	...
	predictKernelMachineClassification(X, N, Kernel, isInteractionTerms, 0, coefficients, n, m, p, Y_hat);
	...
	int csvFile_n1 = m*N+3; // This variable is used to indicate the number of rows with data that will be printed in the .csv file to be created.
	...
	predictKernelMachineClassification(bg_X, N, Kernel, isInteractionTerms, 0, coefficients, (n_ofLinearlySpacedArray*n_ofLinearlySpacedArray), m, p, bg_Y_hat);
    ...
    if(success){
        ...
        WriteToFile(pngdata, length, "main_polynomialKernel (CenyML).png");
